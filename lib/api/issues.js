...
async function create(ctx) {
  ...
  try {
    ...
    const issue = await Issue.create({ title, description, userId: ctx.state.user.id });

    
    //get the plain object so values are not wrapped in sequelize object
    const createdIssue = issue.get({ plain: true })
    console.log("Issue Object", createdIssue);
    ...
    await IssueRevision.create({
      ...
      changes: createdIssue, // Initially, all fields are 'changed' from null
