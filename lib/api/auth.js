...
async function registerSuperAdmin(ctx) {
    ...
    try {
      ...
      const user = await User.create({ username, password, user_group: 'super_admin' });
      ctx.status = 201;
...
async function registerAdmin(ctx) {
  ...
  try {
    ...
    //const hashedPassword = bcryptjs.hashSync(password, 10);
    const user = await User.create({ username, password, user_group: 'admin' });
    ctx.status = 201;
...
async function registerUser(ctx) {
  ...
  try {
    ...
    //const hashedPassword = bcryptjs.hashSync(password, 10);
    const user = await User.create({ username, password, user_group: 'normal' });
    ctx.status = 201;
...
async function login(ctx) {
    ...
    try {
      ...
      const token = jwt.sign({ userId: user.id, userGroup: user.user_group }, config.jwtSecret, { expiresIn: '1h' });
      ctx.status = 200;
